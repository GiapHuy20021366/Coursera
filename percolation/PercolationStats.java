...
public class PercolationStats {
    ...
    public PercolationStats(int n, int trials) {
        if (n <= 0 || trials <= 0) {
            throw new IllegalArgumentException("Invalid argument : n and trails must be larger than zero!");
        }
        ...
        for (int trial = 0; trial < trialCount; ++trial) {
            ...
            while (!percolation.percolates()) {
                int row = StdRandom.uniform(1, n + 1);  // random row
                int col = StdRandom.uniform(1, n + 1);  // random col
                percolation.open(row, col);
            }

            int openSites = percolation.numberOfOpenSites();
            double result = (double) openSites / (n * n);
            this.results[trial] = result;
        }
    }
